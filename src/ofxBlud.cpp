...
//	ofAddListener(ofEvents().windowResized, this, &ofxBlud::windowResized);
...
string ofxBlud::executeFile(std::string filename){
	...
	ofStringReplace(fileloc, "\\", "\\\\");
    luaL_loadfile(luaVM, fileloc.c_str());
...
void ofxBlud::windowResized(int w, int h){
	mutex->lock();
	lua_getglobal(luaVM, "blud");
	lua_getfield(luaVM, -1, "windowResized"); /* function to be called */
	lua_pushnumber(luaVM, w);
	lua_pushnumber(luaVM, h);
	if(lua_pcall(luaVM, 2, 0, 0) != 0){
		printf("error in windowResized: %s\n", lua_tostring(luaVM, -1));
	}
	mutex->unlock();
}
void ofxBlud::windowResized(ofResizeEventArgs &e){
...
"end\n"
"function blud.windowResized(w, h)\n"
"	-- do nothing when initially launching\n"
"end\n"
"function blud.draw()\n"
"	-- do nothing when initially launching\n"
